...
from books.models import Author, Book, Publisher # 템플릿 뷰
from django.views.generic import ListView # 리스트뷰
from django.views.generic import DeleteView # 디테일 뷰
# 레코드 조회를 위한 모델 클래스 임포트
from books.models import Book, Author, Publisher

# books 애플리케이션 첫 화면을 출력
class BooksModelView(TemplateView): # 해당클래스는 TemplateView를 상속하여 정의
    # TemplateView를 사용하는 경우 해당 클래스변수를 오버라이딩 해야한다.
    template_name = 'books/index.html' # 템플릿 파일경로를 지정한다.
    # 함수형 뷰 처럼 render()함수를 통해 반환하지 않아도 자동으로 호출된다.
    
    # 템플릿으로 전달할 데이터가 있는경우 get_context_data()를 오버라이딩 한다.
...
제네릭뷰 중 ListView를 사용하여 리스트 구현
    : ListView 를 상속받는 경우 해당 모델 클래스를 템플릿으로 전달하는 것 만으로
    리스트 구성, 컨텍스트 변수 저장 및 전달이 자동으로 처리된다.
    또한 디폴트로 저장되는 2가지가 있는데
    첫째 컨텍스트 변수로 object_list를 사용한다.
    둘째 템플릿 파일명은 "모델명소문자_list.html"로 저장된다.
'''
...
class BookDetail(DetailView):
    # 1. 테이블로 부터 조회한 값은 object라는 변수명으로 컨텍스트에 저장한다.
