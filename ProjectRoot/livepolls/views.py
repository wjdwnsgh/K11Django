'''
단축함수 : 장고에서 웹 프로그램 개발 시 자주 사용하는 기능등을
...
def vote(request, question_id):
    ...
    question = get_object_or_404(Question, pk=question_id)
    try:
        # detail 페이지에서 선택한 항목을 통해 choice테이블에 레코드를 가져온다.
        selected_choice = question.choice_set.get(pk=request.POST['choice'])
    except (KeyError, Choice.DoesNotExist):
        # 예외발생이 되면 detail페이지를 다시 보여준다. 이때 에러메세지를 전달한다.
        return render(request, 'livepolls/detail.html', {
            'question':question,
            'error_message':"You didn't select a choice.",
        })
    else:
        # try절의 코드가 정상실행되면 사져온 레코드를 통해 votes항목을 +1 업데이트 한다.
        ...
        '''
            reverse(urls의 별칭, 인수)
                ...
                보통은 요청이 있을때 URL을 분석한 후 필요한 view를 호출하지만,
                이 경우에는 별칭을 통해 거꾸로 URL을 만들기 떄문에 reverse라는 이름이 붙었다.
        '''
