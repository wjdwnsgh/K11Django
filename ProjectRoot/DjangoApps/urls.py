...
'''
해당 URLconf는 프로젝트 레벨이므로 livepolls앱의 views 파일(모듈)을 
임포트해야만 접근할 수 있다.
'''
...
urlpatterns = [
    ...
    path('books/', include('books.urls')), # 앱 3 : 도서관리 앱
]
'''
    방법 1의 경우 Url파턴이 변경되면 모든 항목을 수정해야 하므로 불편하다.
    하지만 방법2와 같이 파일을 2개로 나누게되면 하나의 패턴을 위한 네임스페이스가
    생성되는 개념이므로 한번만 수정하면 되서 편리하다.
    즉, livepolls/로 들어오는 모든 요청을 livepolls맵 하위의 urls.py로 전달한다.
'''
